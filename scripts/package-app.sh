...
BIN_ARM64="$(swift build -c release --arch arm64 --build-path "$TMP_ARM64" --show-bin-path)/elz0mbi33"
swift build -c release --arch x86_64 --build-path "$TMP_X86_64"
BIN_X86_64="$(swift build -c release --arch x86_64 --build-path "$TMP_X86_64" --show-bin-path)/elz0mbi33"

rm -rf "$APP_DIR"
mkdir -p "$MACOS_DIR" "$RES_DIR"
cp "$BIN_ARM64" "$MACOS_DIR/elz0mbi33-arm64"
cp "$BIN_X86_64" "$MACOS_DIR/elz0mbi33-x86_64"
